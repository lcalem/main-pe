...
def preprocess_weights_for_loading(layer,
  ...
  """
  print("preprocessing layer %s (origin %s)" % (layer.name, origin))
  ALREADY_SEEN = False
  ...
  def convert_nested_model(weights):
    ...
    Returns:
        A list of weights values (Numpy arrays).
    """
    print("\n converting nested model (in layer %s)!" % layer.name)
    new_weights = []
    # trainable weights
    ...
    for sublayer in layer.layers:
      ...
      print("sublayer %s of %s weights %s, trainable %s, non trainable %s" % (sublayer.name, layer.name, len(sublayer.weights), len(sublayer.trainable_weights), num_weights))

      if len(sublayer.weights) > 0:
        print("weight type %s inner %s" % (type(sublayer.weights), type(sublayer.weights[0])))
      if num_weights > 0:
        new_weights.extend(preprocess_weights_for_loading(
            layer=sublayer,
            weights=weights[:num_weights],
            original_keras_version=original_keras_version,
            original_backend=original_backend,
            origin="convert_non_trainable"))
        weights = weights[num_weights:]
    print("nested model conversion for  %s done" % layer.name)
    return new_weights
  ...
  if layer.__class__.__name__ == 'TimeDistributed':
    ...
    print("layer is a model named %s" % str(layer.name))
  ...
  if layer.__class__.__name__ in conv_layers:
    stop = False
    if layer.name == 'stem1_conv':
        print("PROCESSING STEM1_CONV (%s)" % stop)
        print("HELLO 0 %s vs %s" % (str(K.int_shape(layer.weights[0])), str(weights[0].shape)))
        if ALREADY_SEEN:
            # stop = True
            pass
        ALREADY_SEEN = True
    ...
    if (not stop) and (K.int_shape(layer.weights[0]) != weights[0].shape):
    # if K.int_shape(layer.weights[0]) != weights[0].shape:
      print("HELLO %s" % str(type(layer)))
      print("HELLO 2 %s" % str(layer.name))
      print("HELLO 3 %s vs %s" % (str(K.int_shape(layer.weights[0])), str(weights[0].shape)))
...
def load_weights_from_hdf5_group(f, layers):
  ...
  for k, name in enumerate(layer_names):
    ...
    weight_values = preprocess_weights_for_loading(
        layer, weight_values, original_keras_version, original_backend, origin="load_weights")
